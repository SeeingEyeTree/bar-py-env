...
def main():
    ...
    cats_path = "/sessions/relaxed-nice-maxwell/mnt/outputs/unit_categories_v3.json"
    ...
    out = Path("/sessions/relaxed-nice-maxwell/mnt/outputs/cluster_viewer_v3.html")
...
<title>BAR Cluster Viewer v3</title>
<style>
...
<header>
  <h1>BAR Cluster Viewer v3 <span style="color:var(--muted);font-weight:400;font-size:12px;">— per-tier clustering · weighted role + movementclass features</span></h1>
  <div>Projection:
...
function cc(c){
  // tier-based palette: T1 blues, T2 oranges/pinks/greens, T3 reds/purples
  const tier = Math.floor(c / 100);
  const sub = c % 100;
  const palettes = {
    1: ["#4dabf7","#74c0fc","#22b8cf","#15aabf","#63e6be","#a9e34b","#94d82d"],
    2: ["#ffa94d","#ffd43b","#fcc419","#ff8787","#f783ac","#e599f7","#b197fc","#9775fa","#845ef7"],
    3: ["#ff6b6b","#fa5252","#cc5de8","#ae3ec9","#7950f2"],
  };
  const p = palettes[tier] || PAL;
